...
def center_crop_pad(data, height, width):
    """
...
class NormStats(nn.Module):
    def forward(self, data: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        variance = data.var(dim=1, unbiased=False) + 1e-6
...
class FeatureVarNet_sh_w(nn.Module):
    ...
    def forward(
        ...
        return center_crop_pad(result, 384, 384)
...
class FeatureVarNetBlock(nn.Module):
    ...
    def apply_model_with_crop(self, feature_image: FeatureImage) -> Tensor:
        def run_feature_processor(x):
            return self.feature_processor(x)
    
        if feature_image.crop_size is not None:
            cropped = image_crop(feature_image.features, feature_image.crop_size)
            cropped.requires_grad_(True)
            processed = checkpoint(run_feature_processor, cropped)
            features = image_uncrop(processed, feature_image.features.clone())
        else:
            features = checkpoint(run_feature_processor, feature_image.features)
